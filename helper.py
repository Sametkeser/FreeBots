...
import pytz, datetime, time, timeit, random, sys, ast, re, os, json, subprocess,asyncio, threading, string, codecs, requests, ctypes, urllib, wikipedia, html5lib
...
try:
    ...
    if args.apptype:
        ...
        try:
            enesbyrk = LINE(idOrToken, args.passwd, appType=args.apptype, systemName="CyberTK", channelId=args.channelid, showQr=args.showqr)
            tokenFile.close()
            tokenFile = tokenPath.open('w+')
            tokenFile.write(enesbyrk.authToken)
            tokenFile.close()
        ...
        for appType in listAppType:
            ...
            idOrToken = authToken if authToken else args.email
            try:
                enesbyrk = LINE(idOrToken, args.passwd, appType=appType, systemName=args.systemname, channelId=args.channelid, showQr=args.showqr)
                tokenFile.close()
                tokenFile = tokenPath.open('w+')
                tokenFile.write(enesbyrk.authToken)
                tokenFile.close()
...
if enesbyrk:
    print ('++ Auth Token : %s' % enesbyrk.authToken)
    print ('++ Timeline Token : %s' % enesbyrk.tl.channelAccessToken)
    print ('++ MİD Token : %s' % enesbyrk.getProfile().mid)
...
myAdmin = ["ucb9a7ff919631cabf2d3b25b5d82238b"]
botStart = time.time()
msg_dict = {}
temp_flood = {}
...
#==========================================
           ...
           #                 textx = text.replace(text.split(" ")[0]+" ","")
                        ...
                        if msg._from in myAdmin:               
                          ...
                          if targets:
                            imkhie = 'simple.js gid={} token={}'.format(to, enesbyrk.authToken)
                            for target in targets:
                              imkhie += ' uid={}'.format(target)
                            success = execute_js(imkhie)
                            enesbyrk.inviteIntoGroup(to, [mem])
...
#==========================================
                        ...
                        if msg._from in myAdmin:
                                ...
                                if sender in wait['info']:
                                        ...
                                        if wait["name"][us]["pay"] <= time.time():
                                            ...
                                            try:
                                                def zarifbirbey():                                       
                                                    token = QRV2(to)												
                                                    os.system('screen -S %s -X kill'%us)
                                                    os.system('cp -r tolgbot {}'.format(us))
                                                    os.system('cd {} && echo -n "{}" > token.txt'.format(us, token))
                                                    os.system('screen -dmS {}'.format(us))
                                                    os.system('screen -r {} -X stuff "cd {} && python3 sby.py \n"'.format(us, us))
                                                    contact = enesbyrk.getContact(sender)
                                    ...
                                    mentions(to, ' Merhaba @!\n İletişimini listede göremiyorum. Bot almak istiyorsan @! iletişime geçebilirsin..', [sender, "ucb9a7ff919631cabf2d3b25b5d82238b"])
                                    backupData()
